...
\begin{figure}
   ...
   \text{1. Parse } \sigma_i := (A_i, e_i) \text{ with } A_i := C_i^{\frac{1}{\sk+e_i}} \text{ and } C_i=g_1 \sprod_{k\in\idxSet} h_k^{a_k} \\
            ...
            \text{12. Output } \presentation := (\bar{A_1}, \ldots, \bar{A_n},U, \bar{B}, s, z)
    }
    \end{pcvstack}
...
\begin{figure}
   ...
   \text{1. Parse } \presentation := (\bar{A_1}, \ldots, \bar{A_n},U, \bar{B}, s, z) \\
   ...
   \text{5. Sample } \delta_i \getsr \ZZ_p \text{ of $l_b$ bits} \\
    \text{5. Check } \pairing(\sprod_{i\in[n]} \bar{A_i}^{\delta_i}, \pk) = \sprod_{i\in[n]} \pairing(\bar{B_i}, g_2)^{\delta_i}
    }
    \end{pcvstack}
